...
def break_image_into_shares(img_path, n, r):
    """
    Break an image into `n` shares using polynomial-based Shamir Secret Sharing.
    Args:
    - img_path (str): Path to the image to be encoded.
    - n (int): Total number of shares.
    - r (int): Threshold number of shares required for reconstruction.
    
    Returns:
    - list: A list of file paths to the created shares.
    """
    img_flattened, shape = preprocessing(img_path)
    secret_imgs, imgs_extra = polynomial(img_flattened, n=n, r=r)
    share_paths = []
    
    for i, img in enumerate(secret_imgs):
        share_path = f"share_{i + 1}.png"
        Image.fromarray(img.astype(np.uint8)).save(share_path)
        img_extra = str(list(imgs_extra[i])).encode()
        insert_text_chunk(share_path, share_path, img_extra)
        share_paths.append(share_path)
    
    return share_paths

def reconstruct_image_from_shares(share_paths, r):
    """
    Reconstruct an image from `r` shares using Lagrange interpolation.
    Args:
    - share_paths (list): List of file paths to the shares.
    - r (int): Number of shares to use for reconstruction.
    ...
    input_imgs_extra = []
    for i in range(r):
        img_extra = read_text_chunk(share_paths[i])
        img, shape = preprocessing(share_paths[i])
        input_imgs.append(img)
        input_imgs_extra.append(img_extra)
    
    input_imgs = np.array(input_imgs)
    reconstructed_img = decode(input_imgs, input_imgs_extra, range(r), r=r)
    reconstructed_img = reconstructed_img.reshape(*shape)
